...
## 2-5. 질의개념복습 풀이 이력 (concept_review_attempts)

로그인학생이 푼 질의개념복습 문제·정오답을 저장하고, AI 취약점 분석에 쓰려면 아래 테이블이 필요합니다.

Supabase **SQL Editor**에서 `supabase/migrations/concept_review_attempts.sql` 내용을 실행하세요.
...
### concept_review_attempts (질의개념복습 풀이 이력)
- `student_user_id`, `source_question`, `source_answer`, `quiz_question`, `correct_index`, `user_choice_index`, `is_correct`, `created_at` — 취약점 분석·통계용
